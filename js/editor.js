...
function numberQLine() {
  return new Promise(function (resolve) {
    ...
    qLineQty = Math.floor(qLineQty); //切り捨て
    resolve();
  });
}
...
function checkHit(lane, y, set, margin) {
  if (
    laneSet[lane] < mouseDownX &&
    mouseDownX < laneSet[lane + 1] &&
    y - margin < mouseDownY &&
    mouseDownY < y + margin &&
    set &&
    mouseDown
      ...
      for (let k = 0; k < xLine[i][j].length; k++) {
        if (xLine[i][j][k].note) {
          ...
          if (k < 8) {
            outInfo[count][3] =
              Math.round(
                (j * noteValue + k * note32Value + musicBody * 1000) * 100
              ) / 100;
          } else {
            outInfo[count][3] =
              Math.round(
                (j * noteValue + (k % 8) * note6Value + musicBody * 1000) * 100
              ) / 100;
          }
          count++;
        }
      ...
      for (let j = 0; j < xLine[data[i][1] - 1].length; j++) {
        for (let k = 0; k < xLine[data[i][1] - 1][j].length; k++) {
          if (
            k < 8 &&
            Math.floor(data[i][3]) ==
              Math.floor(j * noteValue + k * note32Value + musicBody * 1000)
          ) {
            ...
            Math.floor(data[i][3]) ==
              Math.floor(
                j * noteValue + (k % 8) * note6Value + musicBody * 1000
              )
          ) {
            xLine[data[i][1] - 1][j][k].note = true;
            flg = true;
